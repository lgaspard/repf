...
if __name__ == '__main__':
    ...
    train_start = [datetime(2020, 3, 27, tzinfo=timezone.utc) +
                   timedelta(days=d) for d in range(1)]
    ...
    solar_stats = pd.read_csv('../resources/csv/solar_power_stats.csv', sep=';', 
    ...
    for START, END, START_FOR, END_FOR in zip(train_start, train_end, test_start, test_end):
        ...
        # Divide Elia's data into training and testing data
        ...
        fit = pystan_model(train_elia['corrected'] * 1e3, AREA_PEAK, ETA,
                           obs_irradiance['GHI'] / 1e3, 
                           for_irradiance['GHI'] / 1e3)
        ...
        prior_MSE, prior_RMSE = compute_metrics(elia_metric['corrected'].values, 
                                                naive_metric['GHI'].values)
        post_MSE, post_RMSE = compute_metrics(elia_metric['corrected'].values, 
                                              mu_for_metric)

        # Performance assessment (remove nigth predictions) for Elia's 
        # predictions
        ...
        baseline_2.drop(baseline_2.index[~baseline_2.index.isin(
            test_elia.index.time)], inplace=True)
        ...
        ax.grid()
        fig.tight_layout()
        fig.savefig('../products/pdf/solar_provincial (START_FOR {}).pdf'.format(
